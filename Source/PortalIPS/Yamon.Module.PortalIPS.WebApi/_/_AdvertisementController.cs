...
//     如果想修改此文件的方法，请在此文件的上级目录中的AdvertisementController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 广告
    /// </summary>
    public partial class _AdvertisementController :BaseController
    {

        public AdvertisementDAL dal = new AdvertisementDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(Advertisement model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        Advertisement newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(Advertisement model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        Advertisement newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<Advertisement> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<Advertisement> modelList = new List<Advertisement>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<Advertisement>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_Advertisement_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            Advertisement model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
