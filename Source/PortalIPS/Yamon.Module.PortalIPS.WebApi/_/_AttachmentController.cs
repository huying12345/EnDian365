...
//     如果想修改此文件的方法，请在此文件的上级目录中的AttachmentController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 附件模型
    /// </summary>
    public partial class _AttachmentController :BaseController
    {

        public AttachmentDAL dal = new AttachmentDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(Attachment model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.AttachmentID;

                    hash["id"] = newId;
                    if(returnData){
                        Attachment newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(Attachment model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.AttachmentID;
                    if(returnData){
                        Attachment newrow = dal.GetModelByID(model.AttachmentID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<Attachment> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"Attachment_InfoGuid":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_Attachment_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            Attachment model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
