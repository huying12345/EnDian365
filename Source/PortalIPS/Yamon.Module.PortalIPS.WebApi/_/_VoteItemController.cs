...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    public partial class _VoteItemController :BaseController
    {
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(VoteItem model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        VoteItem newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(VoteItem model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.VoteItemID;
                    if(returnData){
                        VoteItem newrow = dal.GetModelByID(model.VoteItemID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<VoteItem> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<VoteItem> modelList = new List<VoteItem>();
            switch(filterId)
            {
                    case "VoteItem_VoteID":
                    {
                      modelList = dal.GetEntityListByPage_VoteID(out totalRows,0,page,rows,orderby);
                      break;
                    }
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"VoteItem_VoteID":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_VoteItem_Show")]        
