...
//     如果想修改此文件的方法，请在此文件的上级目录中的PLinksController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 友情链接模型
    /// </summary>
    public partial class _PLinksController :BaseController
    {

        public PLinksDAL dal = new PLinksDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(PLinks model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        PLinks newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(PLinks model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.LinkID;
                    if(returnData){
                        PLinks newrow = dal.GetModelByID(model.LinkID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<PLinks> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<PLinks> modelList = new List<PLinks>();
            switch(filterId)
            {
                    case "PLinks_CategoryID":
                    {
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"PLinks_CategoryID":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_PLinks_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            PLinks model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
