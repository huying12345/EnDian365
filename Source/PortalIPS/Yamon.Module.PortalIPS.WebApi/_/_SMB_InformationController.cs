...
//     如果想修改此文件的方法，请在此文件的上级目录中的SMB_InformationController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 信息内容
    /// </summary>
    public partial class _SMB_InformationController :BaseController
    {

        public SMB_InformationDAL dal = new SMB_InformationDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(SMB_Information model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.InfoGuid;

                    hash["id"] = newId;
                    if(returnData){
                        SMB_Information newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(SMB_Information model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.InfoGuid;
                    if(returnData){
                        SMB_Information newrow = dal.GetModelByID(model.InfoGuid);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<SMB_Information> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<SMB_Information> modelList = new List<SMB_Information>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<SMB_Information>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_SMB_Information_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            SMB_Information model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
