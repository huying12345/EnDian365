...
//     如果想修改此文件的方法，请在此文件的上级目录中的VoteResultController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 投票结果模型
    /// </summary>
    public partial class _VoteResultController :BaseController
    {

        public VoteResultDAL dal = new VoteResultDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(VoteResult model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.VoteResultID;

                    hash["id"] = newId;
                    if(returnData){
                        VoteResult newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(VoteResult model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.VoteResultID;
                    if(returnData){
                        VoteResult newrow = dal.GetModelByID(model.VoteResultID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<VoteResult> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<VoteResult> modelList = new List<VoteResult>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_VoteResult_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            VoteResult model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
