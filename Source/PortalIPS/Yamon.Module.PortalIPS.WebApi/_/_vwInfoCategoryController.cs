...
//     如果想修改此文件的方法，请在此文件的上级目录中的vwInfoCategoryController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 信息内容视图
    /// </summary>
    public partial class _vwInfoCategoryController :BaseController
    {

        public vwInfoCategoryDAL dal = new vwInfoCategoryDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(vwInfoCategory model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        vwInfoCategory newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(vwInfoCategory model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        vwInfoCategory newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<vwInfoCategory> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<vwInfoCategory> modelList = new List<vwInfoCategory>();
            switch(filterId)
            {
                    case "vwInfoCategory_CategoryID":
                    {
                    if (string.IsNullOrEmpty(sort))
                    {
                      orderby = "BackInt asc,ShowTime desc,CreateTime desc";
                    }
                      modelList = dal.GetEntityListByPage_CategoryID(out totalRows,0,page,rows,orderby);
                      break;
                    }
                    case "vwInfoCategory_Deleted":
                    {
                      modelList = dal.GetEntityListByPage_Deleted(out totalRows,0,page,rows,orderby);
                      break;
                    }
                    case "vwInfoCategory_InfoGuid":
                    {
                    if (string.IsNullOrEmpty(sort))
                    {
                      orderby = "BackInt asc,ShowTime desc,CreateTime desc";
                    }
                      modelList = dal.GetEntityListByPage_InfoGuid(out totalRows,0,page,rows,orderby);
                      break;
                    }
                    case "vwInfoCategory_MyAuditInfo":
                    {
                      modelList = dal.GetEntityListByPage_MyAuditInfo(out totalRows,0,page,rows,orderby);
                      break;
                    }
                    case "vwInfoCategory_MyCheckedInfo":
                    {
                      modelList = dal.GetEntityListByPage_MyCheckedInfo(out totalRows,0,page,rows,orderby);
                      break;
                    }
                    case "vwInfoCategory_MyInfo":
                    {
                      modelList = dal.GetEntityListByPage_MyInfo(out totalRows,0,page,rows,orderby);
                      break;
                    }
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"vwInfoCategory_CategoryID":filterId;
            try
            {
                int totalRows = 0;
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<vwInfoCategory>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_vwInfoCategory_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            vwInfoCategory model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
