...
//     如果想修改此文件的方法，请在此文件的上级目录中的GuestBookController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 留言模型
    /// </summary>
    public partial class _GuestBookController :BaseController
    {

        public GuestBookDAL dal = new GuestBookDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(GuestBook model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        GuestBook newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(GuestBook model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        GuestBook newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<GuestBook> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<GuestBook> modelList = new List<GuestBook>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_GuestBook_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            GuestBook model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
