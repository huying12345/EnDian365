...
//     如果想修改此文件的方法，请在此文件的上级目录中的CategoryTypeController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 栏目类型模型
    /// </summary>
    public partial class _CategoryTypeController :BaseController
    {

        public CategoryTypeDAL dal = new CategoryTypeDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(CategoryType model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.TypeName;

                    hash["id"] = newId;
                    if(returnData){
                        CategoryType newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(CategoryType model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.TypeName;
                    if(returnData){
                        CategoryType newrow = dal.GetModelByID(model.TypeName);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<CategoryType> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<CategoryType>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_CategoryType_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            CategoryType model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
