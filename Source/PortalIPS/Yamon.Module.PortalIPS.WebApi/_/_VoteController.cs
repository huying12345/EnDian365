...
//     如果想修改此文件的方法，请在此文件的上级目录中的VoteController重写(override)该方法。
...
namespace Yamon.Module.PortalIPS.WebApi
{
    ...
    /// 投票模型
    /// </summary>
    public partial class _VoteController :BaseController
    {

        public VoteDAL dal = new VoteDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(Vote model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        Vote newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(Vote model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.VoteID;
                    if(returnData){
                        Vote newrow = dal.GetModelByID(model.VoteID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<Vote> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<Vote> modelList = new List<Vote>();
            switch(filterId)
            {
                    case "Vote_CategoryID":
                    {
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"Vote_CategoryID":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"PortalIPS_Vote_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            Vote model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
